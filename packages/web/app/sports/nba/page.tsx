...
export default function NBAPage() {
  ...
  useEffect(() => {
    async function fetchNBAData() {
      try {
        ...
        const [gamesRes, standingsRes] = await Promise.all([
          fetch('/api/sports/nba/games'),
          fetch('/api/sports/nba/standings'),
        ]);
    ...
    const interval = setInterval(fetchNBAData, 30000);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link
                href="/"
                className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-purple-600 bg-clip-text text-transparent hover:from-orange-700 hover:to-purple-700 transition-all"
              >
                Blaze Sports Intel
              </Link>
              <span className="text-gray-400">•</span>
              <h1 className="text-xl font-semibold text-gray-800">NBA</h1>
            </div>
            ...
            {!authLoading && (
              <>
                {authenticated && user ? (
                  <Link
                    href="/profile"
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-600 to-purple-600 text-white rounded-lg hover:from-orange-700 hover:to-purple-700 transition-all"
                  >
                    {user.picture ? (
                      <img
                        src={user.picture}
                  ...
                  <Link
                    href="/login"
                    className="px-6 py-2 bg-gradient-to-r from-orange-600 to-purple-600 text-white rounded-lg hover:from-orange-700 hover:to-purple-700 transition-all font-medium"
                  >
                    Sign In
                  </Link>
                )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
            <h3 className="text-red-800 font-semibold mb-2">Error Loading NBA Data</h3>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <select
                  ...
                  className="px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                >
              ...
              <div className="space-y-8">
                {sortedDivisions.map(division => {
                  const isEastern = ['Atlantic', 'Central', 'Southeast'].includes(division);
                  return (
