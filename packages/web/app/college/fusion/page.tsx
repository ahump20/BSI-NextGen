...
function winDelta(actualWins: number, expectedWins: number | null): string {
  if (!expectedWins && expectedWins !== 0) return '—';
  const diff = actualWins - expectedWins;
  const rounded = Math.round(diff * 10) / 10;
  return `${rounded > 0 ? '+' : ''}${rounded}`;
}

function recordFromStandings(standings: FusionPayload extends { success: true }
  ? FusionPayload['standings']
  : never) {
...
export default async function CollegeFusionPage({
  ...
  const payload = await fetchFusion(params);
  ...
  const { sport, team, standings, analytics, upcomingGame, scoreboard } =
    payload;
    ...
    <main className="di-page">
      ...
      <section className="di-section fusion-grid">
          ...
          <div className="fusion-metrics-row">
            ...
            <div className="fusion-metric">
              ...
              <span className="fusion-metric-sub">
                Pythagorean exponent {pyth?.inputs.exponent ?? '—'}
              </span>
            </div>
            <div className="fusion-metric">
        ...
        <article className="fusion-card fusion-upcoming">
          <h2>{upcomingGame ? 'Next leverage spot' : 'No live game detected'}</h2>
          {!upcomingGame ? (
            <p>
            ...
            <div className="fusion-upcoming-body">
              <div className="fusion-upcoming-teams">
                <div>
                  ...
                  <strong>{upcomingGame.away?.names?.short}</strong>
                ...
                <div>
                  ...
                  <strong>{upcomingGame.home?.names?.short}</strong>
                  <span className="fusion-upcoming-meta">
      ...
      <section className="di-section fusion-footer">
        <p className="fusion-footer-note">
          Scoreboard data powered by your ncaa-api instance; season analytics
          from Blaze&apos;s real NCAA orchestrator. Tune <code>sport</code>,{' '}
          <code>teamId</code>, <code>year</code>, and <code>week</code> via the
          URL query for different teams/slates.
        </p>
      </section>
    </main>
  );
