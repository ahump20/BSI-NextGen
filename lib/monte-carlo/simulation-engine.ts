/**
 * Sandlot Sluggers - Monte Carlo Simulation Engine
 *
 * Statistical simulation engine for Baseball game outcomes
 * Ported from Blaze Sports Intel with adaptations for arcade baseball mechanics
 *
...
export class MonteCarloSimulationEngine {
  ...
  private readonly SPORT_EXPONENTS = {
    ...
    MLB: 1.83,    // Baseball
    ARCADE: 1.80  // Arcade Baseball (slightly lower due to higher variance)
  };
  ...
  private readonly HOME_ADVANTAGE = {
    ...
    ARCADE: 0.05   // 5% boost for home games (arcade balance)
  };
  ...
  private calculatePlayoffProbability(
    ...
    const PLAYOFF_THRESHOLDS = {
      ...
      ARCADE: totalGames * 0.60 // 60% win rate for arcade tournament
    };
  ...
  private calculateChampionshipProbability(
    ...
    const CHAMPIONSHIP_THRESHOLDS = {
      ...
      ARCADE: totalGames * 0.75 // 75%+ win rate for championship
    };
